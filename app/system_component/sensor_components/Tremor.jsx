...
class Tremor extends Component {
    constructor() {
        super();
        this.columns = [
            {
    ...
    getSensorNum(){
        ...
        for(let i=0;i<20;i++){
            tableData.push({
                key:i,
                order:i+1,
                sensor_id:`震颤度传感器${i+1}`,
                associated_device_id:`关联设备${i+1}`,
                ins_ser_record:`震颤度传感器${i+1}`,
                action:[`震颤度传感器${i+1}`,`关联设备${i+1}`],
            });
        }
            ...
            <div>
                ...
                >
                    <div style={{marginBottom:'10px'}}>
                        <label style={{fontSize:'large'}}>震颤度传感器ID:{this.state.checkedSensorId}</label>
